...
```typescript
{
  ...
  // ========================================
  directionScores: {
    expansion: number,      // 확장 (vs 보존)
    preservation: number,
    approach: number,       // 접근 (vs 회피)
    avoidance: number,
    certainty: number,      // 확실성 (vs 탐색)
    exploration: number,
    active: number,         // 능동 (vs 수동)
    passive: number
  },
  directionSummary: {
    primaryAxis: string,    // '확장-접근형'
    description: string
  },
  ...
  // ========================================
  operatingScores: {
    solo: number,           // 독자 (vs 협력)
    collaborative: number,
    depth: number,          // 심화 (vs 확산)
    breadth: number,
    structured: number,     // 구조 (vs 유연)
    flexible: number,
    sprint: number,         // 단거리 (vs 장거리)
    marathon: number
  },
  operatingSummary: {
    primaryStyle: string,   // '독자적 심화형'
    description: string
  },
